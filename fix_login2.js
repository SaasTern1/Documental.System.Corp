...
// The login screen block is completely gone. We need to insert it back.
// Find the right location: right after </div> that closes sidebar-overlay, before <style>@media

const targetInsert = `    <style>
...
const loginBlock = `    <style>
    ...
    <div id="login-screen" style="align-items:stretch; flex-direction:row; justify-content:stretch; display:none;">
...
if (html.includes(targetInsert)) {
    html = html.replace(targetInsert, loginBlock);
    fs.writeFileSync('index.html', html, 'utf8');
    console.log("✅ Login screen rebuilt correctly!");
} else {
    console.log("Pattern not found. Showing lines around the target area:");
    const lines = html.split('\n');
    console.log(lines.slice(36, 45).join('\n'));
}
