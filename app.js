...
try {
    ...
    // Selector de prioridad (Solo admins)
    ...
    // Bloqueo Inteligente de Documento Original (Solo para revisiones de SGC y Dueños en Fases Avanzadas)
    ...
    if (stepIdx >= 2 && !esDuenio && !esAdminSGC) { 
        setHtml('m-file-link', `<span style="color:#64748b; font-size:13px; font-style:italic;"><span class="material-icons-round" style="font-size:14px; vertical-align:middle;">lock</span> Documento original bloqueado por confidencialidad en esta fase.</span>`);
    } else {
    ...
    // Unificación del Paso 4 (SGC Final) y la Publicación
    if (stepIdx === 3 && puedeGestionarSGC && activo) {
        setDisplay('m-panel-final-sgc', 'block'); setVal('m-final-cod', s.cod_ref || "");
        setDisplay('m-actions', 'none'); // Esconde botones normales para obligar a usar el panel de publicación
    }
...
// Puede que estén editando un final ya existente sin subir otro archivo, así que validamos
...
// Si el índice es 3 (Paso 4 SGC), se aprueba final aquí mismo
...
let nameF = esAdminSGC ? "Reporte_SGC_Completo_Con_Tiempos" : "Reporte_Solicitudes"; let wb = XLSX.utils.book_new(); let ws = XLSX.utils.json_to_sheet(dataExport); XLSX.utils.book_append_sheet(wb, ws, "Datos_Filtrados"); XLSX.writeFile(wb, `${nameF}.xlsx`);
};
